...
if [ -f /jffs/.OEM/.showname ];then
	SHOW_NAME=`cat /jffs/.OEM/.showname`
else
	duty_date="2008-2014"
	if [ -f /jffs/.OEM/.duty_date ];then
		duty_date=`cat /jffs/.OEM/.duty_date`
	fi
	corporation=`$SHOWSYSINFO 2>/dev/null|awk -F':' '/Company Name/{print $2}' 2>/dev/null`
	SHOW_NAME="$duty_date $corporation"
fi

[ -z "$corporation" ]&&corporation="Autelan"
HOST_NAME="${corporation:-Autelan}"

productType=`$SHOWSYSINFO 2>/dev/null|awk -F':' '/Device Type/{print $2}' 2>/dev/null|cut -b 1-4 2>/dev/null`
...
DEVTXC_wifi0=`cat $DEVICE_INFO_XML 2>/dev/null | awk -f $XML_GET_DATA_BY_OPTION_AWK  option1=wifi0 option2=txchain`
DEVTXC_wifi1=`cat $DEVICE_INFO_XML 2>/dev/null | awk -f $XML_GET_DATA_BY_OPTION_AWK  option1=wifi1 option2=txchain`
WAPI_ENABLE=enable
...
BRLAN=default
FACTORY_RESET=/sbin/sysfactoryreset
ACKTIMEOUT=/usr/sbin/acktimeout
