...
uci_init(void)
{
    ...
    if (NULL==umc.uci.ctx) {
        debug_uci_error("[um]:open uci context failed");
...
package_open(char *name)
{
    ...
    if (err) {
        debug_uci_error("[um]:open uci package(%s) failed", name);
    } else {
...
__load_intf(char *ifname, struct blob_attr *tb[], int count, struct list_head *head)
{
    ...
    if (NULL==intf) {
        debug_uci_error("[um]:create intf(%s) failed", ifname);
...
load_radio(struct uci_section *s, struct blob_attr *tb[], int count, struct list_head *head)
{
    ...
    if (1 != os_sscanf(ifname, "radio%d", &radioid)) {
        debug_uci_error("[um]:load radio failed(bad name:%s)", ifname);
...
load_wlan(struct uci_section *s, struct blob_attr *tb[], int count, struct list_head *head)
{
    ...
    if (p) {
        ...
        debug_uci_trace("[um]:load wlan without ifname");
    ...
    wlanid = num % 100;
#else
    os_sscanf(ifname, "ath.%d-%d", &radioid, &wlanid);
    ...
    debug_uci_trace("[um]:load %s", ifname);
...
um_uci_load(void)
{
    ...
    if (NULL==wireless) {
        err = -EINVAL1;
        debug_uci_error("[um]:wireless open failed(%d)", err);
    ...
    if (err<0) {
        debug_uci_error("[um]:wireless load failed(%d)", err);
