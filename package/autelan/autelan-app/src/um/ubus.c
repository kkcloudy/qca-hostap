...
pushuser(struct apuser *user, bool init, char *name)
{
	...
	um_user_add_string(UM_USER_IP, user->stdevinfo.ipaddr);
	...
	um_user_add_bool(UM_USER_PORTAL_ENABLE, user->portal.enable);
    ...
    um_user_add_string(UM_USER_DEVTYPE, user->stdevinfo.devtype);
    um_user_add_string(UM_USER_HOSTNAME, user->stdevinfo.hostname);
    um_user_add_string(UM_USER_OSTYPE, user->stdevinfo.ostype);
    um_user_add_string(UM_USER_DEVMODEL, user->stdevinfo.devmodel);
    um_user_add_string(UM_USER_CPUTYPE, user->stdevinfo.cputype);
    um_user_add_u32(UM_USER_DELAYTIME, user->delaytime);
    um_user_add_u32(UM_USER_PACKETLOSS, (uint32_t)user->packetloss);
    um_user_add_u32(UM_USER_RX_RETR, (uint32_t)user->rx_retr);
    um_user_add_u32(UM_USER_TX_RETR, (uint32_t)user->tx_retr);
    um_user_add_u32(UM_USER_UP_PACKET, user->wifi.tx.packets);
    um_user_add_u32(UM_USER_DOWN_PACKET, user->wifi.rx.packets);
...
static void
pushdevinfo(struct apuser *user)
{
    void *handle;
...
insert_or_remove_cb(struct apuser *user, char *event)
{
    if (user->local) {        
    	...
    	debug_ubus_trace("[um]:%s user(%s)", event, um_macstring(user->mac));
    }
...
void um_ubus_devinfo_cb(struct apuser *user)
{
    if (um_is_ev_enable(devinfo))
    {
        pushdevinfo(user);
        notify("um.devinfo");
    }
...
update_cb(struct apuser *old, struct apuser *new)
{
    ...
    debug_ubus_trace("[um]:um.update user(%s)", um_macstring(old->mac));
...
restart(void)
{
    debug_ubus_trace("[um]:do nothing, now!");
...
add_object(struct ubus_object *obj)
{
	...
	if (err) {
		debug_ubus_error("[um]:Failed to publish object '%s': %s\n", obj->name, ubus_strerror(err));
	}
...
add_subscriber(struct ubus_subscriber *subscriber)
{
	...
	if (err) {
		debug_ubus_error("[um]:Failed to register subscriber(%s)", ubus_strerror(err));
	}
...
um_ubus_connect(char *path)
{
	...
	if (NULL==umc.ctx) {
	    debug_ubus_error("[um]:connect ubus failed");
...
um_ubus_init(char *path)
{
	...
	err = add_object(&umc.obj.object);
    if (err<0) {
        return err;
	}

    err = add_subscriber(&umc.obj.subscriber);
    if (err<0) {
        return err;
	}
    
    debug_ubus_trace("[um]:ubus init");
