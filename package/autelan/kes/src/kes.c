...
//yuanding add for marvell 6710
#ifdef MARVELL_6710
static void *kes_va_start = NULL;
#endif
//yuanding add end
//
...
int print_msg_to_kes_traps(const char * fmt,...)
{
    ...
    if(msg_size > (KES_TRAPS_BLOCK_SIZE - KES_MEM_HEADER_LEN - kes_traps_offset- traps_count* 4*1024))
    {
        traps_count ++;
...
static ssize_t kes_flag_proc_write(struct file *flip, const char __user *buff, size_t len, loff_t *ppos)
{
...
static int kes_flag_proc_read(struct seq_file *m, void *v)
{
    unsigned char tmp[8] = {0};
...
static int kes_flag_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, kes_flag_proc_read, NULL);
}
...
#ifdef MARVELL_6710        //yuanding add for marvell 6710 , 2013-06-24

    if( !request_mem_region(TRAPS_START_ADDR, KES_MEM_SIZE,"KES") ) {
        printk(KERN_INFO "request mem fail\n");
        return -1;
    }
    
    kes_va_start = ioremap_nocache(TRAPS_START_ADDR, KES_MEM_SIZE);
    if( kes_va_start == NULL )
    {
        printk(KERN_INFO "ioremap kes memory fail!\n");
        return -1;
    }

    kes_traps_addr = kes_va_start;
    kes_debug_addr = kes_va_start + KES_TRAPS_BLOCK_SIZE;
    kes_dmsg_addr  = kes_va_start + KES_TRAPS_BLOCK_SIZE + KES_DEBUG_BLOCK_SIZE;
...
/******************************************************************
 Function:
     kes_mem_addr_free
 Description:
     unmap the memory for kes ( marvell 6710 only)
 ...
 DATE:
     2013-06-24
 *****************************************************************/
static void kes_mem_addr_free(void)
{
    if( kes_va_start != NULL)
    {
        iounmap(kes_va_start);
        release_mem_region(TRAPS_START_ADDR, KES_MEM_SIZE);
        kes_va_start = NULL;
        printk(KERN_INFO "kes mem released.\n");
    }
}
#endif
...
static const struct file_operations kes_flag_fops = {
	.owner		= THIS_MODULE,
	.open		= kes_flag_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
	.write		= kes_flag_proc_write,
};
...
static int kes_proc_init(void)
{
    ...
    kes_flag_entry = proc_create(KES_FLAG_NAME,0666,NULL,&kes_flag_fops);
    if(!kes_flag_entry)
    {
        printk(KERN_INFO "kes create %s error.\n", KES_FLAG_NAME);
...
#ifdef PC018
	if((kes_debug_header->isenable[6] == '0' ||kes_debug_header->isenable[6]== ' ')&&(0x80000000U & *(volatile unsigned int *)KSEG1ADDR(0x18060008)))
	{
		if(kes_debug_print_handle)
		{
		   kes_debug_print_handle(KERN_EMERG "The last reset was due to a watchdog timeout \n");
		}
            if(kes_debug_print_flag_handle)
            {
                kes_debug_print_flag_handle("z");
            }
	}
#endif
...
/******** yuanding add for marvell 6710 2013-06-24 ********/
#ifdef MARVELL_6710
    kes_mem_addr_free();
#endif
/********* yuanding add end ***********************/
