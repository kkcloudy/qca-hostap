...
#define WAIT_RX_INTERVAL 10000
    ...
    if (vap->init_in_progress && OS_NON_SLEEPABLE()) {
        ieee80211_note (vap,"%s(): init in progress...Exiting!!!\n", __func__);
        error = -1;
        goto out;
    }
    ...
    while (vap->init_in_progress) {
        if (waitcnt >= OSIF_MAX_STOP_VAP_TIMEOUT_CNT) {
            ieee80211_note (vap,"%s(): Timeout for start response event!!!\n", __func__);
            error = -1;
            goto out;
        }
        schedule_timeout_interruptible(OSIF_STOP_VAP_TIMEOUT);
        waitcnt++;
        ieee80211_note (vap,"%s(): Init in progress. Delay vap_stop\n", __func__);
    }
