...
struct ieee80211_iter_arg {
    ...
    wlan_if_t vap;
    u_int32_t flag;
    struct ieee80211_node *nodes[IEEE80211_MAX_AID_DEF];
};
...
_ieee80211_free_node(struct ieee80211_node *ni)
{
    ...
    if((ni == vap->iv_bss) && (vap->iv_node_count > 1)) {
        ...
        if(ni->ni_ic != NULL) {
            printk("%s  nodes still present in nt \n\r",__func__);
            ieee80211_dump_alloc_nodes(ni->ni_ic);
        }
#endif
    }
#if UNIFIED_SMARTANTENNA
...
/*Added-Begin:pengdecai for 11ac station timeout*/
	...
	if(!ic->ic_is_mode_offload(ic)){
        ni->ni_inact_reload = ni->ni_vap->iv_inact_run;
	}else {//for 11ac iv_inact_run is set 300 instead of 300/IEEE80211_INACT_WAIT 
...
static void ieee80211_node_iter(void *arg, struct ieee80211_node *ni)
{
    ...
    if (itr_arg->count < ic->ic_num_clients) {
        /* increment the ref count so that the node is not freed */
        itr_arg->nodes[itr_arg->count] = ieee80211_ref_node(ni);
    }
...
#else
		struct ieee80211com *ic = ni->ni_ic;
		if(ic->ic_is_mode_offload(ic)){ //pengdecai added for 11ac station timeout
			inact_time = ni->ni_vap->iv_inact_run / IEEE80211_INACT_WAIT ;
		}else {
			inact_time = ni->ni_vap->iv_inact_run;
		}
#endif
