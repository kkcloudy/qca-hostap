...
/*Begin: zhaoyang1 modified for 11ac traffic limit 2014-03-31*/
#define OL_IEEE80211_TL_ENQUEUE_RX_EXTRA(_tl_cq, _peer, _tid) do {  \
			(_tl_cq)->cq_rx_tail->ex_peer = _peer;					 \
			(_tl_cq)->cq_rx_tail->ex_tid = _tid;		   \
			(_tl_cq)->cq_rx_tail = (_tl_cq)->cq_rx_tail->ex_next;  \
		} while (0) 


#define OL_IEEE80211_TL_DEQUEUE_RX_EXTRA(_tl_cq, _peer, _tid) do {  \
        _peer = (_tl_cq)->cq_rx_head->ex_peer;          \
        _tid     = (_tl_cq)->cq_rx_head->ex_tid;              \
        (_tl_cq)->cq_rx_head = (_tl_cq)->cq_rx_head->ex_next; \
    } while (0)
...
int ol_ieee80211_tl_vap_cache_enqueue_rx(struct ieee80211vap *vap, wbuf_t wbuf, 
    void* peer, unsigned tid); //zhaoyang1 modified for 11ac traffic limit 2014-03-31
