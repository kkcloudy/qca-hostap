                                              ...
                                              int ext_chan_offset)
{
    ...
    ICM_ASSERT(extension_channel != 0);
       ...
       for (i = 0; i < pchlist->count; i++) {
           if ((pchlist->ch[i].channel >= start_channel) &&
               ...
               if ((pchlist->ch[i].channel != ctrl_channel) &&
                   ...
                   pchlist->ch[i].used_as_secondary_40++;
               }
           }
       }
...
static ICM_EVENT_DESCRIPTOR_T const*
    icm_get_event_descriptor(int cmd,
                             ...
                             ICM_EVENT_DESCRIPTOR_ADJ_T *desc_adj)
{
    static int num_ioc_desc = IW_ARRAY_LEN(we_ioc_descriptors);
    static int num_ev_desc = IW_ARRAY_LEN(we_ev_descriptors);
 ...
 * Return       : success/failure
 */
void icm_get_ieee_chaninfo(ICM_INFO_T* picm)
{
    struct ieee80211req_chaninfo chans;
 ...
 * Return       : success/failure
 */
int icm_update_channel_ieee_info(ICM_INFO_T* picm, int freq,
    const struct ieee80211_channel *c)
{
