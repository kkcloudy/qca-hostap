				...
				char *data, int len)
{
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		wpa_printf(MSG_DEBUG, 
			"atheros_wireless_event_wireless: receive netlink msg cmd: %d", 
			iwe->cmd); // zhaoyang1 modifies for binding sock with wlan interface 2015-04-03
...
atheros_wireless_event_init(struct atheros_driver_data *drv)
{
	...
	wpa_printf(MSG_DEBUG, "netlink init, iface: %s ifindex: %d\n",
			drv->iface, drv->ifindex); // zhaoyang1 modifies for binding sock with wlan interface 2015-04-03
...
atheros_init(struct hostapd_data *hapd, struct wpa_init_params *params)
{
	...
	/* Cao Jia add netlink sock to NETLINK_ROUTE */
	...
	if (params->bridge[0]) {
		...
		drv->sock_recv = l2_packet_init(drv->iface, NULL, ETH_P_EAPOL,
						handle_read, drv, 1);
		if (drv->sock_recv == NULL)
			goto bad;
	} else
...
static int atheros_br_add_ip_neigh(void *priv, u8 version,
...
static int atheros_br_delete_ip_neigh(void *priv, u8 version,
					 const u8 *ipaddr)
{
#ifdef CONFIG_LIBNL3_ROUTE
	struct i802_bss *bss = priv;
...
const struct wpa_driver_ops wpa_driver_atheros_ops = {
	...
	.br_add_ip_neigh = atheros_br_add_ip_neigh,
	.br_delete_ip_neigh = atheros_br_delete_ip_neigh,
#ifdef CONFIG_IEEE80211R
