	...
	if (l2->l2_hdr) {
		...
		if (ret < 0)
			wpa_printf(MSG_ERROR, "l2_packet_send - send: %s",
				   strerror(errno));
		else // zhaoyang1 modifies for hostap debug 2015-04-03
			wpa_printf(MSG_DEBUG, "%s send eapol packet successfully\n", __func__);
	} else {
...
static void l2_packet_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	// zhaoyang1 modifies for hostap debug 2015-04-03
	wpa_printf(MSG_DEBUG, "l2_packet_receive receive eapol packet\n");
	...
	void *rx_callback_ctx, int l2_hdr)
{
	...
	// zhaoyang1 modifies for hostap debug 2015-04-03
	wpa_printf(MSG_DEBUG, "%s, ifname: %s ifindex: %d raw fd: %d protocol: 0x%x\n",
			__func__, l2->ifname, l2->ifindex, l2->fd, protocol);
