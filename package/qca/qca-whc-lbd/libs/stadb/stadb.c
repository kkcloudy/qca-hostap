    ...
    if (entry) {
        ...
        if (reason == stadb_rssiUpdateReason_probe) {
            ...
            if (lastServingESS == LBD_ESSID_INVALID ||
                ...
                stadbEntryRecordProbeRSSI(entry, bss, rssi,
                                          stadbState.config.probeMaxInterval);
            } else {
                // Do not need to notify RSSI observer since no RSSI change
                return entry;
            }
        } else {
            stadbEntryRecordRSSI(entry, bss, rssi);
        }
    ...
    if (entry) {
        ...
        if (btmStatus != wlanif_cap_unchanged) {
            if (btmStatus == wlanif_cap_enabled) {
                stadbEntryUpdateIsBTMSupported(entry, LBD_TRUE, &btmChanged);
            }
            else {
                stadbEntryUpdateIsBTMSupported(entry, LBD_FALSE, &btmChanged);
            }
        }
        ...
        // Update PHY capability info and populate BSSes from same ESS
        // on association
        if (isAssoc && assocChanged) {
            ...
            stadbEntryPopulateBSSesFromSameESS(entry, bss, phyCapInfo);
        }
